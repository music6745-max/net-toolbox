...
const P107_TEMPLATE_URL = "https://kaigo-okane.booth.pm/items/8424688";
const P108_PACK_URL = "https://kaigo-okane.booth.pm/items/8424970";
...
export default function KaigoCareManagerFirstConsultMemoPage() {
  ...
  const result = useMemo(() => {
    ...
    const beforeConsult = [
      ...
      "かかりつけ医、通院頻度、服薬、お薬手帳の保管場所を確認する",
      ...(livingAlone
        ? ["一人暮らしのため、連絡が取れない時の確認順と訪問できる人を仮決めする"]
        : []),
      ...(needsCertification
        ? ["介護認定の申請状況、申請予定、認定調査で伝えたいことを整理する"]
        : []),
      ...(hospitalDischargeSoon
        ? ["退院予定日、退院後の生活場所、病院から言われている注意点を確認する"]
        : []),
    ];
    ...
    const familyRoles = [
      ...
      `訪問の付き添い: 現在の訪問頻度は${visitFrequency}として、増やせるか確認する`,
      "費用記録: 交通費、立替、親本人の支払いを分けて残す",
      ...(familyNotAligned
        ? ["未決事項: きょうだい間で意見が違う点を、決めることと保留することに分ける"]
        : []),
    ];
    ...
    return { beforeConsult, familyRoles, memoText, questions };
  }, [
    concerns,
    familyNotAligned,
    hospitalDischargeSoon,
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      <header className="mb-6">
        ...
        <p className="mt-3 text-sm leading-relaxed text-muted">
          ケアマネや地域包括支援センターへ初めて相談する前に、生活状況、困りごと、家族の役割、聞きたいことを整理します。
      ...
      <section className="rounded-xl border border-card-border bg-card-bg p-5 sm:p-6">
                ...
                {(Object.keys(concernLabels) as ConcernKey[]).map((key) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        setConcerns((current) => ({ ...current, [key]: event.target.checked }))
                      }
                    />
                    {concernLabels[key]}
                  </label>
            ...
            <div className="rounded-lg bg-background p-4">
              ...
              <label className="mt-2 flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(event) => setNeedsCertification(event.target.checked)}
                />
                介護認定の申請も相談したい
              </label>
              <label className="mt-2 flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={hospitalDischargeSoon}
                  onChange={(event) => setHospitalDischargeSoon(event.target.checked)}
                />
                入院中または退院が近い
              </label>
              <label className="mt-2 flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={familyNotAligned}
                  onChange={(event) => setFamilyNotAligned(event.target.checked)}
                />
                家族の意見がまだまとまっていない
              </label>
            </div>
        ...
        <p className="mt-3 text-xs leading-relaxed text-muted">
          このツールは相談前の整理用です。医療、介護制度、契約、費用、税務、相続の判断は、自治体、地域包括支援センター、医療機関、専門窓口へ確認してください。
        </p>
      </section>
      ...
      <KaigoToolCta
        ...
        description="相談前に伝えることを整理したら、親のこと整理ナビの記事で進め方を確認し、BOOTHのテンプレートへ残せます。"
        links={[
          {
            label: "親のこと整理ナビで診断する",
            href: KAIGO_NAVI_URL,
            eventName: "kaigo_navi_click",
            position: "care_manager_tool",
            variant: "primary",
          },
          { label: "初回相談の進め方を読む", href: GUIDE_URL, eventName: "guide_click", position: "care_manager_guide" },
          {
            label: "ケアマネ初回相談テンプレートを見る",
            href: P107_TEMPLATE_URL,
            eventName: "booth_click",
            position: "single_template",
          },
          {
            label: "入院・退院・ケアマネ相談パックを見る",
            href: P108_PACK_URL,
            eventName: "booth_click",
            position: "hospital_care_manager_pack",
          },
        ]}
      />
      ...
      <ToolFAQSection
        ...
        howTo={[
          "相談の目的と、気になっている生活場面を選びます。",
          "一人暮らし、認定申請、退院予定、家族の意見差などの条件を選びます。",
          "生成された相談前メモ、家族の役割案、聞きたいことをコピーして相談準備に使います。",
        ]}
        faqs={[
          {
            question: "このツールだけで介護サービスを決められますか？",
            answer:
              "決められません。家族が相談前に状況を整理するためのツールです。具体的なサービス利用や制度判断は、ケアマネ、地域包括支援センター、自治体窓口へ確認してください。",
          },
          {
            question: "本人がサービス利用を嫌がっていても使えますか？",
            answer:
              "使えます。本人が嫌がっていること、家族が困っていること、今は困っていないことを分けて相談材料にする用途です。無理に結論を出すためのものではありません。",
          },
          {
            question: "個人情報を入力してもよいですか？",
            answer:
              "入力しないでください。住所、保険証番号、病名、薬名、口座情報、暗証番号などは扱いません。具体情報は手元資料として管理し、必要な場面で窓口の案内に沿って確認してください。",
          },
          {
            question: "家族の意見がまとまっていなくても相談できますか？",
            answer:
              "相談できます。結論ではなく未決事項として持っていけば十分です。誰が連絡係か、誰が記録係か、次回までに何を確認するかを仮で置くと話し合いやすくなります。",
          },
        ]}
      />
